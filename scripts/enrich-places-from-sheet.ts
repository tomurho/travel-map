...
function printHelp() {
  console.log(`Enrich Capture rows from Google Places into Review.

Usage:
  pnpm enrich:places -- --sheet-id <SHEET_ID> --max-api-calls 1 --confirm-live-api

Required environment:
  GOOGLE_PLACES_API_KEY      Used for Google Places Text Search.

Behavior:
  Reads Capture, processes only intakeStatus = Ready,
  appends candidate rows to Review, marks Capture rows Enriched,
  appends API usage rows, and never writes to Published.`);
}
